<?php
    ...
    // Decode old-style encoded rows (backward compat for data inserted before the fix).
    // New rows store the real category in the category column directly.
    ...
    if ($method === 'POST') {
        ...
        // Store actual category in the category column.
        // For 'others', prepend the custom label to the description.
        $dbCat  = $category;
        ...
        try {
            ...
            $stmt->bind_param("idsss", $userId, $amount, $dbCat, $dbDesc, $date);
            $stmt->execute();
